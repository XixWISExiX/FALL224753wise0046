...
```
 ...
 `We can also see that MPG follows the empirical rule well through this histogram.`
 ```{r}
# X-axis grid
...
```
 ...
 7. Is the Empirical rule valid in this case? Why? `Yes because the Empirical rule was within the 95% range (95% and up)`
 `in this case. If the data for example was in the 80% range, then the`
 `Empirical rule would be invalid in this case.`
