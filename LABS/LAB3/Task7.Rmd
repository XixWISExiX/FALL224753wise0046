...
inputPanel(
  ...
  selectInput("plotg", "Choose plot type",
      ...
      # )
      list(`Points` = c("Points alone"),
           `Other Options` = c("SLR line", "Points with line segments")
      )
    )
)
...
# g+ggtitle("Height Vs BHDiameter") + theme(plot.title = element_text(hjust = 0.5))
  ...
  g = ggplot(spruce.df, aes(x = BHDiameter, y = Height, colour=BHDiameter)) + geom_point() + ggtitle("Height Vs BHDiameter") + theme(plot.title = element_text(hjust = 0.5))
  glm1 = g + geom_smooth(method="lm")
  glm2 = g + geom_line()
  
  if(input$plotg == "Points alone") print(g)
  
  if(input$plotg == "SLR line") print(glm1)
  
  if(input$plotg == "Points with line segments") print(glm2)
